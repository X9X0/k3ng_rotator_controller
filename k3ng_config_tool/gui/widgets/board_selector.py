...
from ...boards.board_database import BoardDatabase, ArduinoBoard
...
class BoardSelectorWidget(QWidget):
    ...
    def _load_boards(self):
        ...
        for board in boards:
            ...
            if 'mega' in board.board_id.lower():
                item.setText(f"⭐ {board.name} (Recommended)")
    ...
    def _format_board_details(self, board: ArduinoBoard) -> str:
        ...
        <table style='width: 100%; border-collapse: collapse;'>
            ...
            <tr>
                ...
                <td style='padding: 5px;'>{board.pins.digital_count}</td>
                <td style='padding: 5px;'>Pins {board.pins.digital_range[0]}-{board.pins.digital_range[1]}</td>
            </tr>
            <tr style='background-color: #f9f9f9;'>
                <td style='padding: 5px;'><strong>Analog Pins</strong></td>
                <td style='padding: 5px;'>{board.pins.analog_count}</td>
                <td style='padding: 5px;'>{board.pins.analog_range[0]} to {board.pins.analog_range[1]}</td>
            </tr>
            <tr>
                <td style='padding: 5px;'><strong>PWM Pins</strong></td>
                <td style='padding: 5px;'>{len(board.pins.pwm_pins)}</td>
                <td style='padding: 5px;'>{', '.join(map(str, board.pins.pwm_pins[:10]))}{', ...' if len(board.pins.pwm_pins) > 10 else ''}</td>
            </tr>
            <tr style='background-color: #f9f9f9;'>
                <td style='padding: 5px;'><strong>Interrupt Pins</strong></td>
                <td style='padding: 5px;'>{len(board.pins.interrupt_pins)}</td>
                <td style='padding: 5px;'>{', '.join(f'Pin {pin} (INT{num})' for pin, num in list(board.pins.interrupt_pins.items())[:5])}</td>
            </tr>
        </table>

        <h3>Communication</h3>
        <p><strong>I2C:</strong> SDA=Pin {board.pins.i2c['sda']}, SCL=Pin {board.pins.i2c['scl']}</p>
        <p><strong>Serial Ports:</strong> {len(board.pins.serial_ports)} available</p>
    ...
    def _on_select_clicked(self):
        ...
        self.current_label.setText(f"Current Board: {board.name}")
        self.current_label.setStyleSheet("""
            QLabel {
                padding: 10px;
        ...
        QMessageBox.information(
            ...
            f"Pin validation will now use {board.name} specifications.\n"
    ...
    def get_selected_board(self) -> Optional[ArduinoBoard]:
